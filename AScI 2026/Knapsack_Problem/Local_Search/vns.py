...
def main():
    ...
    for r in range(100, 3100, 100):
        ...
        # Executa VNS
        print("Executando VNS...")
        start_vns = time.time()
        ls.vns(sol,3)
        end_vns = time.time()
        #sol.print()
        tempo_vns = end_vns - start_vns
        val_vns = sol.obj             # valor da solucao VNS
        ...
        if mod.model.status == OptimizationStatus.OPTIMAL:
            ...
            gap = 100 * (val_opt - val_vns) / val_opt
            resultados.append([seed, int(val_opt), tempo_opt, int(val_vns), gap, tempo_vns])
        else:
            resultados.append([seed, '-', tempo_opt, int(val_vns), 'N/A', tempo_vns])

    # Impressão da tabela formatada
    print("\n\t\t\tSolução Ótima\t\tVNS")
    print("seed\t\tFO\ttime (s)\tFO\terro\ttime (s)")
    for linha in resultados:
        seed, fo_opt, t_opt, fo_vns, erro, t_vns = linha
        fo_opt_str = f"{fo_opt}".ljust(6)
        t_opt_str = f"{t_opt:.2f}".ljust(8) if isinstance(t_opt, float) else f"{t_opt}".ljust(8)
        fo_vns_str = f"{fo_vns}".ljust(6)
        erro_str = f"{erro:.3f}%" if isinstance(erro, float) else erro
        print(f"{seed}\t{fo_opt_str}{t_opt_str}\t{fo_vns_str}{erro_str}\t{t_vns:.2f}")
