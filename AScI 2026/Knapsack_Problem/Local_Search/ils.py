...
def main():
    ...
    for r in range(100, 3100, 100):
        ...
        end_ils = time.time()
        #sol.print()
        tempo_ils = end_ils - start_ils
        val_ils = sol.obj             # valor da solucao ILS
        ...
        if mod.model.status == OptimizationStatus.OPTIMAL:
            ...
            gap = 100 * (val_opt - val_ils) / val_opt
            resultados.append([seed, int(val_opt), tempo_opt, int(val_ils), gap, tempo_ils])
        else:
            resultados.append([seed, '-', tempo_opt, int(val_ils), 'N/A', tempo_ils])

    # Impressão da tabela formatada
    print("\n\t\t\tSolução Ótima\t\tILS")
    print("seed\t\tFO\ttime (s)\tFO\terro\ttime (s)")
    for linha in resultados:
        seed, fo_opt, t_opt, fo_ils, erro, t_ils = linha
        fo_opt_str = f"{fo_opt}".ljust(6)
        t_opt_str = f"{t_opt:.2f}".ljust(8) if isinstance(t_opt, float) else f"{t_opt}".ljust(8)
        fo_ils_str = f"{fo_ils}".ljust(6)
        erro_str = f"{erro:.3f}%" if isinstance(erro, float) else erro
        print(f"{seed}\t{fo_opt_str}{t_opt_str}\t{fo_ils_str}{erro_str}\t{t_ils:.2f}")
