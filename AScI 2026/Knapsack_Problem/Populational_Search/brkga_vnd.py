...
class CLocalSearch():
    ...
    def vnd(self, sol, strategy='first'):
        ...
        # Ao final, garanta que sol recebe a melhor solução encontrada
        sol.copy(solstar)
    ...
    def brkga(self, inst, population_size=50,
                ...
                max_generation=200,
                best_sol=None):  # <-- Adicionado best_sol
        ...
        if best_sol is not None:
            best_sol.copy(pop[0])  # pop[0] é o melhor indivíduo após o sort final
...
def main():
    ...
    for r in range(100, 3100, 100):  # Agora cobre 30 seeds
        ...
        ls.brkga(inst,
            ...
            mutant_population_size=30,
            elite_inheritance_prob=60,
            max_generation=200,
            best_sol=best_sol)  # <-- Passe o argumento aqui
        
        # --- Busca local VND ---
        busca_local = CLocalSearch()
        busca_local.vnd(best_sol, strategy='first')
        # -----------------------
